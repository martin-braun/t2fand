...
test-execution success. The package-release reconciliation in `REQ-082`,
`DEC-034`, and `H-022` is historical and superseded by current `REQ-083`,
`DEC-035`, and `H-023`: verified `PKGBUILD` metadata targets `2.0.0-dev-1`
(`pkgver=2.0.0-dev`, `pkgrel=1`). The README disclaimer requirement is defined
by `REQ-084`; it is not yet checked-in. Build, publication, runtime, and
documentation-edit outcomes remain unknown.
...
### Current package-release and documentation requirements

| ID        | Requirement                                                                                                                                                                                                                                                                                                                    | Acceptance evidence                                                                             |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| `REQ-082` | Historical package-release target `2.0.0-1` (`pkgver=2.0.0`, `pkgrel=1`).                                                                                                                                                                                                                                                      | superseded by `REQ-083`; prior checked-in contract evidence retained                            |
| `REQ-083` | Set the target Arch package release to `2.0.0-dev-1` (`pkgver=2.0.0-dev`, `pkgrel=1`). Retain the current package name, architecture, license, dependencies, local sources, and three-file OpenRC payload contract. This requirement defines checked-in package metadata and does not claim publication or a successful build. | verified checked-in `PKGBUILD`; build, publication, and runtime outcomes unknown                |
| `REQ-084` | Put a prominent disclaimer immediately below the `README.md` title and before other substantive content: `This version is under active development and should not be installed.` The disclaimer must precede installation or service-use guidance and must not be weakened by surrounding documentation.                       | current README lacks the disclaimer; documentation edit required; documentation outcome unknown |
...
target release `2.0.0-dev-1`, retains its checked-in metadata, retains
`linux-t2`, `python`, and `git`, and includes `util-linux` for
`/usr/bin/logger`. The package payload is exactly the daemon, OpenRC init
script, and conf.d file. `backup=()` preserves `/etc/conf.d/t2fand`. No package
rename, systemd directory, unit, post-install service action, or guessed syslog
daemon dependency is allowed.
...
`2.0.0-dev-1` (`pkgver=2.0.0-dev`, `pkgrel=1`). Runtime dependencies are
`linux-t2`, `python`, and `util-linux`; `git` remains a build dependency. No
specific syslog daemon is required. The local package-source list and skipped
checksums are static metadata; build and artifact provenance outcomes remain
unknown.
...
`REQ-078`–`REQ-080` rename, the benchmark prerequisite in
`REQ-081`/`IF-034`/`DEC-033`/`H-021`, and the current package/documentation
requirements in `REQ-083`–`REQ-084`, alongside the other existing surfaces. They
establish source/static definitions and test-definition presence only; they do
not establish runtime behavior or any hardware, service, package, staging, or
...
10. Current static/package tests must prove `OPENRC_CONFDIR`, the three staged
    ...
    assignment, `backup=()`, release `2.0.0-dev-1`, and no service action.
...
13. Current documentation review must prove the affected operator surfaces
    ...
    with its exact unavailable-path error/no-side-effect contract. README must
    also place the active-development installation disclaimer from `REQ-084` at
    the top, without contradicting this contract.
...
The checked-in `PKGBUILD` provides evidence for the current `2.0.0-dev-1`
package metadata; the prior `2.0.0-1`, `2.0.1-1`, and `2.0.1-2` handoffs remain
historical evidence. The checked-in static package validation checks that the
local-source `PKGBUILD` delegates staging through
`make DESTDIR="$pkgdir" install`, that the Makefile stages the three `REQ-075`
files and modes, and that the target package release is `2.0.0-dev-1` with no
alternate selector or service payload. Test execution remains unknown. The
default fake fixture creates a T2 fan at `devices/a/b/c/d/APP0001:00`, so this
static fan-discovery definition aligns with `REQ-052`. No test pass or
implementation-runtime claim is made from those definitions.
...
- `DEC-024` (historical; superseded by `DEC-034`; checked-in package
  definitions; staging, build, and runtime unknown): ship the administrator
  conf.d file as the third exact package payload, preserve it with `backup=()`,
  and set the package release to `2.0.1-2`.
...
- `DEC-033` (authoritative checked-in benchmark decision; checked-in source and
  ...
  are not expanded.
- `DEC-034` (historical; superseded by `DEC-035`): target package release
  `2.0.0-1` (`pkgver=2.0.0`, `pkgrel=1`).
- `DEC-035` (authoritative checked-in package-release decision; verified
  `PKGBUILD`; build, publication, and runtime outcomes unknown): target package
  release is `2.0.0-dev-1` (`pkgver=2.0.0-dev`, `pkgrel=1`). The README must
  also carry the active-development disclaimer in `REQ-084`; that documentation
  edit is not yet checked in.
...
- `H-022`: superseded package-release reconciliation: verified `PKGBUILD`
  metadata restored the target release to `2.0.0-1`; the prior `2.0.1-2` target
  is historical. Publication, build, and runtime outcomes remain unknown.
- `H-023`: current package/documentation reconciliation: verified `PKGBUILD`
  metadata targets `2.0.0-dev-1` (`pkgver=2.0.0-dev`, `pkgrel=1`), and the
  README contract requires an active-development disclaimer before substantive
  content. The disclaimer edit, publication, build, and runtime outcomes remain
  unknown.
...
| `Q-023` | Whether the current `2.0.0-dev-1` package builds, stages, publishes, or operates successfully is unknown; checked-in `PKGBUILD` metadata is verified.                                                                                                                                                                                                                                                                                                                                                                         |
