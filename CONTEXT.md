...
- `PKGBUILD` names local `t2fand`, `t2fand.initd`, `t2fand.confd`, and
  ...
  `2.0.0-dev-1` (`pkgver=2.0.0-dev`, `pkgrel=1`), and `util-linux` for
  `/usr/bin/logger`.
...
- `.github/workflows/build.yml` provides package build, artifact, and release
  automation.
- README/CONTEXT synchronization follows REQ-051 and REQ-074: README begins with
  the active-development/no-install disclaimer and remains concise operator
  onboarding for prerequisites, installation, both control modes, the conf.d
  override, compact/full telemetry, CLI options, benchmark timestamps, the
  stress-ng PATH prerequisite, OpenRC operation, safety, and project-native
...
### 2.0.0-dev-1 release transition

- The checked-in package target moved from historical `2.0.1-2` to `2.0.0-dev-1`
  (`pkgver=2.0.0-dev`, `pkgrel=1`). The package name, architecture, license,
  dependencies, local sources, and three-file OpenRC payload remain unchanged.
  `PKGBUILD` verifies the metadata; build, publication, and runtime outcomes
  remain unknown.
- README now places the required active-development/no-install disclaimer
  immediately below its title, before installation or service guidance. This
  verifies documentation placement only; no install or service result is
  claimed.
